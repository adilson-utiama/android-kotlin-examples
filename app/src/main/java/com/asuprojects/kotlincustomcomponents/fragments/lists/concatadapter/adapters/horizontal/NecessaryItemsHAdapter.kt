...
import com.asuprojects.kotlincustomcomponents.databinding.ViewholderNecessaryItemsHorizontalBinding

class NecessaryItemsHAdapter : RecyclerView.Adapter<NecessaryItemsHAdapter.NecessaryItemsVH>() {
    ...
    class NecessaryItemsVH(private val binding: ViewholderNecessaryItemsHorizontalBinding): RecyclerView.ViewHolder(binding.root){
        fun bind(){}

        fun getBinding(): ViewholderNecessaryItemsHorizontalBinding = binding
    }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): NecessaryItemsVH {
        return NecessaryItemsVH(
                ViewholderNecessaryItemsHorizontalBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        )
    }
    ...
    override fun onBindViewHolder(holder: NecessaryItemsVH, position: Int) {
        ...
        holder.getBinding().recyclerHorizontalItems.adapter = itemAdapter

        holder.getBinding().vhNiHViewDetails.setOnClickListener {
            listener?.onClickDetails("Adapter Necessary Items Horizontal - absolute_adapter_position: ${holder.absoluteAdapterPosition}, binding_adapter_position: ${holder.bindingAdapterPosition}")
        }
